...
class Seed_Common {
	...
	static function memcpy_byte2int(&$dst, &$src, $length) {
		$iLen = (int)($length / 4);
		for($i=0; $i<$iLen; $i++) {
			Seed_Common::byte_to_int($dst, $i, $src, $i*4);
		}
	}
	...
	static function int_to_byte(&$dst, $dst_offset, &$src, $src_offset) {
		Seed_Common::int_to_byte_unit($dst, $dst_offset, $src[$src_offset]);
	}
...
class KISA_SEED_CBC {
	...
	static function chartoint32_for_SEED_CBC(&$in, $inLen) {
		...
		for($i=0;$i<$len;$i++)
		{
			Seed_Common::byte_to_int($data, $i, $in, $i*4);
		}
		return $data;
	}
	...
	{	
		...
		$pInfo->encrypt = $enc;
		Seed_Common::memcpy_byte2int($pInfo->ivec, $pbszIV, 16);
		$pInfo->last_block_flag = $pInfo->buffer_length = 0;

		$ABCD[KISA_SEED_CBC::$ABCD_A] = Seed_Common::get_byte_to_int($pbszUserKey, 0*4);
		$ABCD[KISA_SEED_CBC::$ABCD_B] = Seed_Common::get_byte_to_int($pbszUserKey, 1*4);
		$ABCD[KISA_SEED_CBC::$ABCD_C] = Seed_Common::get_byte_to_int($pbszUserKey, 2*4);
		$ABCD[KISA_SEED_CBC::$ABCD_D] = Seed_Common::get_byte_to_int($pbszUserKey, 3*4);
	...
	static function SEED_CBC_Process( &$pInfo, &$in, $inLen, &$out, &$outLen ) {
		...
		if( KISA_ENC_DEC::KISA_ENCRYPT == $pInfo->encrypt ) {
			...
			Seed_Common::memcpy_int2int( $pInfo->ivec, $pdwXOR, $pdwXOR_offset, KISA_SEED_CBC::BLOCK_SIZE_SEED );
			Seed_Common::arraycopy_system($in, $in_offset, $pInfo->cbc_buffer, 0, $pInfo->buffer_length);
		}
		else {
			$pdwXOR = $pInfo->ivec;
			$in_offset = 0;
			...
			$outLen = $nCurrentCount - KISA_SEED_CBC::BLOCK_SIZE_SEED;
			Seed_Common::memcpy_int2int( $pInfo->ivec, $pdwXOR, $pdwXOR_offset, KISA_SEED_CBC::BLOCK_SIZE_SEED );
			Seed_Common::memcpy_int2int( $pInfo->cbc_last_block, $out, $out_offset-KISA_SEED_CBC::BLOCK_SIZE_SEED_INT, KISA_SEED_CBC::BLOCK_SIZE_SEED );
		}
	...
	static function SEED_CBC_Close( &$pInfo, &$out, $out_offset, &$outLen ) {
		...
		if( KISA_ENC_DEC::KISA_ENCRYPT == $pInfo->encrypt ) {
			...
			for( $i = $pInfo->buffer_length; $i<KISA_SEED_CBC::BLOCK_SIZE_SEED; $i++ ) {
				Seed_Common::set_byte_for_int($pInfo->cbc_buffer, $i, $nPaddngLeng);
			}
			...
			$nPaddngLeng = Seed_Common::get_byte_for_int($pInfo->cbc_last_block, KISA_SEED_CBC::BLOCK_SIZE_SEED-1);
			if( $nPaddngLeng > 0 && $nPaddngLeng <= KISA_SEED_CBC::BLOCK_SIZE_SEED )
			{
				for($i = $nPaddngLeng;$i>0;$i--)
				{
					Seed_Common::set_byte_for_int($out, $out_offset-$i, 0);
				}
				$outLen = $nPaddngLeng;
			}
			else
	...
	static function SEED_CBC_Encrypt( &$pbszUserKey, &$pbszIV, &$message, $message_offset, $message_length ) {
		...
		Seed_Common::arraycopy_system($message, $message_offset, $pbszPlainText, 0, $message_length);
		...
		Seed_Common::arraycopy($newpbszPlainText, $pbszPlainText, $nPlainTextLen);
		...
		Seed_Common::arraycopy($pbszCipherText, $cdata, $nRetOutLeng+$nPaddingLeng);
	...
	static function SEED_CBC_Decrypt( &$pbszUserKey, &$pbszIV, &$message, $message_offset, $message_length ) {
		...
		Seed_Common::arraycopy_system($message, $message_offset, $pbszCipherText, 0, $message_length);
		...
		Seed_Common::arraycopy($newpbszCipherText, $pbszCipherText, $nCipherTextLen);
		...
		{
			$cdata = KISA_SEED_CBC::int32tochar_for_SEED_CBC( $outbuf, $nRetOutLeng-$nPaddingLeng );
			Seed_Common::arraycopy($pbszPlainText, $cdata, $nRetOutLeng-$nPaddingLeng);
			$message_length = $nRetOutLeng-$nPaddingLeng;
			
			if($message_length < 0)
				$message_length = 0;
			$result = array_pad(array(), $message_length, 0);
			Seed_Common::arraycopy_system($pbszPlainText, 0, $result, 0, $message_length);
		}
		$data = null;
		$cdata = null;
		$outbuf = null;
...
$g_bszUser_key = "D1,E3,33,8F,08,17,79,F1,E9,F3,94,37,0A,D4,05,89";
...
function dec_string($enctxt){
	$dectxt_len = strlen($enctxt);
	$dec_txt = "";
	if($dectxt_len>0){
		for($e=0;$e<($dectxt_len/2);$e++){
			$s=$e*2;
			$dec_ary[$e] = substr($enctxt, $s, 2);
		}


		$dec2 = decrypt($g_bszIV, $g_bszUser_key, implode($dec_ary, ","));
		$decExp	= explode(",", $dec2);
		for($f=0;$f<count($decExp);$f++){
			$dec_txt .= hexTostr($decExp[$f]);
		}
...
function enc_string($nomal_string){
	$userarray	= str_split($nomal_string);

	for($t=0;$t<count($userarray);$t++){
		$userhex[$t]	= StrToHex($userarray[$t]);
	}

	$userenc = encrypt($g_bszIV, $g_bszUser_key, implode($userhex, ","));
	$userenc = strtolower(str_replace(",","",$userenc));
